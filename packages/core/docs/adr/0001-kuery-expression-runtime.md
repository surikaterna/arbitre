...
Kuery owns the bounded JSON expression AST, immutable profile, compilation, evaluation, lazy `if`, and expression dependency extraction. Arbitre owns shorthand lowering at registration, live resolution across scope/bindings/namespaces, ordered writes, scheduling, TMS, effects, and custom stages. `$inc` treats only an inspectable absent terminal own property as zero; uninspectable properties, existing non-finite numbers, and every non-number (including `undefined` and `null`) are errors, as are non-finite amounts and results. `$merge` accepts only exact `Object.prototype` or null-prototype data objects with enumerable own data properties and safe string keys. It shallow-copies without invoking getters, preserving the RHS prototype for a missing target and the target prototype otherwise, while materializing writable properties for later stages. Scope mutations prepare a descriptor-only copy-on-write path, including fresh traversed ordinary/null-prototype objects, arrays, the selected namespace root, and the store-set root, before atomically swapping one trusted reference. Arrays retain length, holes, and index behavior; inherited properties are absent. Class instances and uninspectable objects are supported only as opaque leaves, so nested writes through them fail, and untouched opaque aliases remain shared. A valid-shape proxy is materialized; reflection traps may have unavoidable external effects, but failed preparation cannot change Arbitre state, cache, or provenance. Provenance is prepared before the store swap and appended afterward with rollback on unexpected failure; rule reversion and checkpoint restore prepare all roots before committing. Checkpoint and provenance clones recursively preserve supported null prototypes and cycles. Invalid `$inc`/`$merge` writes expose only redacted allowlisted rule/path/reason diagnostics.
